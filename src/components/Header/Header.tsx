...
const Header: FC = () => {
  ...
  const getHeaderContent = (user:User | null) =>{
      if (user){
        const { username } = user
        let { image } = user
        if (image === null) image = 'https://static.productionready.io/images/smiley-cyrus.jpg' 
  return (<ul className = {classes.header__nav}>
  ...
  <li>
    ...
    >
      ...
      return (<ul>
        <li  className={classes.header__nav_item}>
          <Link  className={classes.header__nav_link} to="/sign-in">Sign In</Link>
        </li>
        <li  className={classes.header__nav_item}>
          <Link  className={classes.header__nav_link} to="/sign-up">Sign Up</Link>
        </li>
      </ul>)
} 
